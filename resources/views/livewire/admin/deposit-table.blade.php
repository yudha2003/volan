...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group flex-wrap mb-2">
            ...
            <button wire:click="otomatis" class="btn btn-primary ">Otomatis</button>
            <button wire:click="manual" class="btn btn-primary ">Manual</button>
        </div>
        <div class="btn-group flex-wrap mb-2">
        ...
        <div class="card">
            <h5 class="card-header"><i class="mdi mdi-credit-card me-2"></i>Deposit Users</h5>
            <div class="card-body">
                ...
                <div class="table-responsive">
                                        ...
                                        @if ($row->status == 'pending')
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu" style="cursor:pointer">
                                                    ...
                                                    <li><a class="dropdown-item" href="#"
                                                            wire:click="ubahStatus('canceled', '{{ $row->id }}')">Batalkan</a>
                                                    </li>
                                                </ul>
                                            </div>
                                    ...
                                    <td class="text-nowrap">
                                        ...
                                        @if ($row->status == 'pending')
                                            <button href="javascript:;"
                                                wire:click.prevent='konfirmasi({{ $row->id }})'
                                                class="btn btn-soft-success btn-sm btn-rounded font-size-13"><i
                                                    class="mdi mdi-check-circle me-1"></i>Konfirmasi</button>
                                            <button href="javascript:;"
                                        ...
                                        @else
                                            <button href="javascript:;"
                                                class="btn btn-soft-danger btn-sm btn-rounded font-size-13" disabled><i
                                                    class="mdi mdi-close-circle-outline me-1"></i>Batalkan</button>
                                        @endif
                                    </td>
                                </tr>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            Menampilkan
                            @if ($history->count() > 0)
                                {{ $history->firstItem() }} sampai
                                {{ $history->lastItem() }} dari
                                {{ $history->total() }} data
                            @endif
                        </div>
                        <div class="col-md-6">
...
<script>
    window.addEventListener('show-confirmation', event => {
        Swal.fire({
            ...
            text: "Anda akan mengkonfirmasi deposit ini!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                Livewire.emit('konfirmasi')
            }
        })
    })
    window.addEventListener('layananConfirm', event => {
        Swal.fire(
            'Terhapus!',
            'Deposit berhasil di konfirmasi.',
            'success'
        )
    });
    window.addEventListener('failedConfirm', event => {
        Swal.fire(
            'Gagal!',
            'Data gagal di konfirmasi.',
            'error'
        )
    });
    window.addEventListener('show-delete-confirmation', event => {
        Swal.fire({
            title: 'Apakah Anda Yakin?',
            text: "Anda akan merubah status deposit!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    window.addEventListener('swal:confirm', event => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Livewire.emit('ubahStatus', event.detail.status, event.detail.id)
            }
        });
    });
</script>
